...
sub parse_amplicon_file {
	...
	my ($markerdata, $markers) = read_amplicon_data($amplicons_file, 'markers');
...
# 	}
	...
	my ($sampledata, $samples) = read_amplicon_data($amplicons_file, 'samples');
...
# 	# my ($amplicondata, $amplicons) = read_amplicon_data($amplicons_file, 'amplicons');
...
# 	}
	...
	my $alleledata = read_amplicon_data($amplicons_file, 'alleles');
...
# 	my $paramsdata;
	my $paramsdata = read_amplicon_data($amplicons_file, 'params');
...
# if ($md5 eq 'b525199f535e212452d8413b651050f6'){
# print '';
# }
			my @allele_names;
			my $max_ident = 0;
				...
				if ($options->{'alignment'} =~ /blast/) {
					...
					if ($max_ident < $allele_align_data->{$md5}[$j]{'IDENT'}) {
						...
						if (defined($options->{'amplitaxo'})){
							@allele_names = sprintf("%s"."aligned=%d;ident=%d;total=%d;",$allele_align_data->{$md5}[$j]{'NAME'},$allele_align_data->{$md5}[$j]{'ALIGNED'},$allele_align_data->{$md5}[$j]{'IDENT'},$len_seq);
						} else {
						...
						$max_ident = $allele_align_data->{$md5}[$j]{'IDENT'};
						if (defined($options->{'amplitaxo'})){
							push(@allele_names, sprintf("%s"."aligned=%d;ident=%d;total=%d;",$allele_align_data->{$md5}[$j]{'NAME'},$allele_align_data->{$md5}[$j]{'ALIGNED'},$allele_align_data->{$md5}[$j]{'IDENT'},$len_seq));
						} else {
...
# 						if ($output_aligned) {
# 							$allele_align_info->{$md5}{$allele_align_data->{$md5}[$j]{'NAME'}}{'aligned'} = $allele_align_data->{$md5}[$j]{'ALIGNED'};
# 						}
# 						if ($output_ident) {
# 							$allele_align_info->{$md5}{$allele_align_data->{$md5}[$j]{'NAME'}}{'ident'} = $allele_align_data->{$md5}[$j]{'IDENT'};
# 						}
					} else {
						last;
					}
...
# 				push(@{$cluster_data},{'md5' => $dominant_md5, 'aligned_seq' => $dominant_seq, 'depth' => $amplicon_depths{$dominant_md5}, 'identity' => 100, 'errors' => ''});
	...
	# 					}
				...
				} #ends non singletons processing
...
sub read_multifile_amplicons {

	my ($multifile,$options,$max_depth) = @_;

	my ($type, $verbose) = (undef, 1);
	...
	my @file_list = decompress($multifile,$type,$tmp_dir);
	...
	foreach my $file (@file_list) {
		...
		my @path_names = reverse(split("\/",$file));
		my $marker_name;
		# Gives the name of the internal folder to the marker (if it is not the temporal dir)
		if ($path_names[1] ne $tmp_dir_name){
			$marker_name = convert_to_alphanum($path_names[1]);
		# If not gives the name of the multifile without extension
		} elsif ($multifile =~ /.+\/(.+?)\./ || $multifile =~ /(.+?)\./) {
			$marker_name = $1;
		# If not gives the full name of the multifile
		} else {
			$marker_name = $multifile;
		}
...
# }
			if (defined($sample_data->{'depth_amplicon'})){
				...
				if (!defined($max_depth) || $max_depth >= $amplicon_depths->{$marker}{$sample}){
...
# 					}
					...
					my $ratio = 0.5*($max_depth/$amplicon_depths->{$marker}{$sample});
...
# 			}
			...
			foreach my $md5 (@amplicon_major_variants) {
				...
				my $seq = $marker_seq_data->{$marker_name}{$md5}{'seq'};
				my $freq = $amplicon_seq_data->{$marker_name}{$sample_name}{$md5}{'freq'};
				# Aligns low freq variants to the major one to detect the kind of sequencing errors
...
# 						$major_errors->{'indels'} += scalar @$insertions + scalar @$deletions;
# 						$major_errors->{'substitutions'} += scalar @$substitutions;
